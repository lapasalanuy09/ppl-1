                ...
                <div class="navbar-btn">
                    ...
                    <form class="navbar-form navbar-left"method="GET" action="/pendaftaran-mahasiswa">
                        ...
                        <li class="dropdown">
                            ...
                            @if ($profil->pendaftaran != null && $profil->pendaftaran->profil != null)
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown"><img
                                src="{{ asset('storage/' . $profil->pendaftaran->profil) }}" class="img-circle"
                                alt="Avatar" style="width: 30px; height: 30px">
                                <span>{{ auth()->user()->nama }}</span> <i
                                class="icon-submenu lnr lnr-chevron-down"></i>
                            </a>
                            @else
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown"><img
                                src="{{ asset('template/assets/img/user.png') }}" class="img-circle" alt="Avatar">
                                <span>{{ auth()->user()->nama }}</span> <i class="icon-submenu lnr lnr-chevron-down"></i>
                            </a>
                            @endif
                            @endif
                            ...
                            <ul class="dropdown-menu">
                                @if (Auth::user()->role == 'user')
                                <li><a href="{{ route('myprofil', ['id' => Auth::user()->id]) }}"><i
                                    class="lnr lnr-user"></i> <span>My
                                        Profile</span></a></li>
                                        @endif
